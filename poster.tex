...
{
		\ifpdf
			\includegraphics[width=\paperwidth+0.7pt]{background3.pdf}\\
		\else
			\includegraphics[width=\paperwidth+0.7pt]{background3.eps}\\
		\fi
};
\end{tikzpicture}%
}
...
\headerbox{1. CSMA/CA}{name=contention,column=0,below=motivation}{
...
\headerbox{2. Basic CSMA/ECA}{name=hysteresis,column=0,below=contention}{
...
\headerbox{3. CSMA/ECA + hysteresis and fair share}{name=fullECA,column=1,span=1,below=motivation}{

CSMA/ECA is totally distributed, that means that the number of nodes is unknown to all contenders. So, to make it possible to achieve a collision-free state with more than $CW_{\min}/2$ contenders, CSMA/ECA:
\begin{itemize}
...
\end{itemize}
We called this measure \emph{hysteresis}. With hysteresis some nodes may have larger $B_{d}$ than others.~This~unfairness issue is averted by:
...
\path
	\ifpdf
		(0,0) node{\includegraphics[width=0.9\linewidth]{csma_eca_different_backoff_short.pdf}}
	\else
		(0,0) node{\includegraphics[width=0.9\linewidth]{csma_eca_different_backoff_short.eps}}
	\fi
	(0,-1.7) node {\smaller Example CSMA/ECA behavior with hysteresis and fair share ($CW_{\min}=16$).};
\end{tikzpicture}
\end{center}
...
\headerbox{4. Conclusions, Future plans and Acknowledgements}{name=future,column=1,below=fullECA}{
...
% 
 \bibitem{CSMA_ECA}
Barcelo, J. and Lopez-Toledo, A. and Cano, C. and Oliver, M.
